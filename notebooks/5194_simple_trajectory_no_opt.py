...
CO2_AFOLU_VAR = "Emissions|CO2|AFOLU"
...
for marker, params in markers_to_process.items():
    ...
    # --- build total-CO2 trajectory at World level, then derive FFI = total - AFOLU ---
    # This mirrors the pattern in fossil_co2_storyline_functions.extend_co2_for_scen_storyline:
    # total = fossil + AFOLU  →  shape total to net-zero  →  fossil = shaped_total - AFOLU
    # (see also extend_from_start_to_stop_with_value for the subtraction step)
    nz_idx = int(nz_yr - t_vals[0])

    region_vals_src = source_co2.index.get_level_values("region")
    var_vals_src    = source_co2.index.get_level_values("variable")

    ffi_world_row = source_co2[(region_vals_src == "World") & (var_vals_src == CO2_FFI_VAR)]
    ffi_world_vals = ffi_world_row.values[0].astype(float) if len(ffi_world_row) else np.zeros(len(t_vals))

    # AFOLU lives in source_other (co2_mask excludes it); pull from df_continuous directly
    afolu_mask  = (
        (df_continuous.index.get_level_values("scenario") == src_scen)
        & (df_continuous.index.get_level_values("model")    == src_model)
        & (df_continuous.index.get_level_values("variable") == CO2_AFOLU_VAR)
        & (df_continuous.index.get_level_values("region")   == "World")
    )
    afolu_world_rows = df_continuous[afolu_mask]
    if len(afolu_world_rows):
        afolu_world_vals = afolu_world_rows.values[0].astype(float)
    else:
        print(f"  WARNING: no World AFOLU row found for source {params['source_marker']} — treating as zero")
        afolu_world_vals = np.zeros(len(t_vals))

    # Shape total CO2 to net-zero using the sigmoid/exponential algorithm
    total_co2        = ffi_world_vals + afolu_world_vals
    function_total   = total_co2[: t_extend + 1]

    # Transition type is determined by the total-CO2 derivative (one label per marker)
    deriv     = get_derivative_using_spline(function_total, t_vals, t_extend)
    sig_deriv = get_sigmoid_derivative(0.0, function_total[-1], sigmoid_shift=0)
    t_type    = (
        "exponential"
        if (deriv < 0 and sig_deriv < 0 and abs(deriv) > abs(sig_deriv))
        else "sigmoid"
    )
    print(f"  Transition type (total CO2): {t_type}")

    total_co2_extended = do_simple_sigmoid_or_exponential_extension_to_target(
        function_total, t_vals, t_extend,
        target=0.0,
        sigmoid_shift=0,
        sigmoid_len=sig_len,
    )
    # Enforce hard net-zero for total CO2
    total_co2_extended[nz_idx:] = 0.0

    # Derive World FFI = shaped total - AFOLU (AFOLU unchanged, same as extend_from_start_to_stop_with_value)
    ffi_world_extended          = total_co2_extended - afolu_world_vals
    ffi_world_extended[nz_idx:] = 0.0

    # Departure-year FFI total (World) for fractional-share calculation
    ffi_dep = ffi_world_vals[t_extend]

    # --- extend each CO2 FFI row by scaling its departure-year share ---
    cf_co2_rows = []
    summary_rows = []
    ...
    for idx_tuple, row in source_co2.iterrows():
        ...
        # Fractional share of this row in World-total FFI at departure_year
        share = (values_all[t_extend] / ffi_dep) if ffi_dep != 0.0 else 0.0

        # Build extended array: keep original pre-departure, then scale the World FFI trajectory
        extended          = values_all.copy()
        extended[t_extend:] = share * ffi_world_extended[t_extend:]
        extended[nz_idx:]   = 0.0
        ...
        print(f"    [share={share:+.3f}]  {var_short[:45]:45s}  region={region}")
        summary_rows.append({"variable": var_short, "region": region, "transition": t_type, "share": share})
...
for marker, params in markers_to_process.items():
    ...
    src_afolu_row = df_fair_final[
        (df_fair_final["scenario"] == source_marker) & (df_fair_final["variable"] == "CO2 AFOLU")
    ]
    cf_ffi_row = df_fair_final[
        (df_fair_final["scenario"] == marker) & (df_fair_final["variable"] == "CO2 FFI")
    ]
    cf_afolu_row = df_fair_final[
        (df_fair_final["scenario"] == marker) & (df_fair_final["variable"] == "CO2 AFOLU")
    ]

    # Source: FFI and total CO2
    if len(src_ffi_row):
        src_ffi_vals = src_ffi_row[year_cols_f].values.flatten()
        ax.plot(years_fair, src_ffi_vals,
                label=f"{source_marker} CO2 FFI", color=source_color, linewidth=2)
        if len(src_afolu_row):
            src_total_vals = src_ffi_vals + src_afolu_row[year_cols_f].values.flatten()
            ax.plot(years_fair, src_total_vals,
                    label=f"{source_marker} total CO2", color=source_color, linewidth=1.5,
                    linestyle=":", alpha=0.7)

    # CF marker: FFI and total CO2
    if len(cf_ffi_row):
        cf_ffi_vals = cf_ffi_row[year_cols_f].values.flatten()
        ax.plot(years_fair, cf_ffi_vals,
                label=f"{marker} CO2 FFI", color=marker_color, linewidth=2, linestyle="--")
        if len(cf_afolu_row):
            cf_total_vals = cf_ffi_vals + cf_afolu_row[year_cols_f].values.flatten()
            ax.plot(years_fair, cf_total_vals,
                    label=f"{marker} total CO2", color=marker_color, linewidth=1.5,
                    linestyle="-.", alpha=0.7)
    ...
    ax.set_xlabel("Year")
    ax.set_ylabel("CO2 (Mt CO2/yr)")
    ax.set_title(
        f"{marker}: total CO2 net-zero trajectory  [departure {dep_yr} → net-zero {nz_yr}]\n"
        f"(dotted = total CO2, dashed = CO2 FFI, AFOLU unchanged)"
    )
    ax.legend()
