...
DATA_DIR_FRANKENSTEIN = Path().resolve().parent / "data" / "vl-extensions-inputs"
OUTPUTS_DIR = Path().resolve().parent / "outputs" / "vl-extensions-outputs"
...
df_frankenstein = pd.read_csv(DATA_DIR_FRANKENSTEIN / "VL_frankenstein-full.csv", index_col=[0, 1, 2, 3, 4])
df_frankenstein_afolu = pd.read_csv(DATA_DIR_FRANKENSTEIN / "VL_frankenstein_AFOLU-full.csv", index_col=[0, 1, 2, 3, 4])
scenarios_complete_global = pd.concat([df_frankenstein, df_frankenstein_afolu])
...
scenarios_regional = pd.read_csv(
    DATA_DIR_FRANKENSTEIN / "VL_frankenstein-FFCO2-sector_region.csv",
...
# %%
# AFOLU section
def calculate_afolu_extensions(scenarios_complete_global, history, cumulative_history_afolu):
    """
